...
print("Pokrećem Test 06 – provjera redoslijeda prolaska vozila...")
...
izlazna_mapa = "rezultati/smjer_kretanja"
os.makedirs(izlazna_mapa, exist_ok=True)

izlazni_video = os.path.join(
    izlazna_mapa,
    "test06_smjer_kretanja_final.mp4"
)


# =========================================================
# LINIJE DOBIVENE KALIBRACIJOM
# =========================================================
# Ako si nakon ovoga dobio nove koordinate kalibracijom,
# zamijeni samo ovaj blok svojim najnovijim vrijednostima.

linije = {
    "T1-L1": ((316, 483), (396, 503)),
    "T1-L2": ((253, 536), (338, 555)),

    "T2-L1": ((526, 403), (572, 408)),
    "T2-L2": ((495, 440), (558, 451)),

    "T3-L1": ((700, 435), (768, 434)),
    "T3-L2": ((692, 381), (740, 381)),

    "T4-L1": ((901, 591), (991, 573)),
    "T4-L2": ((863, 525), (922, 510)),
}
...
# Prethodna pozicija svakog ID-a
prethodne_pozicije = {}

# Pamti kada je vozilo aktiviralo L1 određene trake
aktivacije_l1 = {}

# Sprječava višestruku aktivaciju iste linije istim ID-em
aktivirane_linije_po_id = set()

# Vozila koja su već završila ispravan prolaz L1 -> L2
zavrseni_id = set()

# Brojanje ispravnih prolazaka po trakama
brojaci = {
    "Traka 1": 0,
    "Traka 2": 0,
    "Traka 3": 0,
...
while True:
    ...
    # Povećana rezolucija i niži confidence omogućuju
    # bolju detekciju udaljenih i manjih vozila.
    results = model.track(
        frame,
        persist=True,
    ...
    if boxes.id is not None:
        ...
        for box, track_id, cls_id in zip(
            ...
            if track_id in prethodne_pozicije:
                ...
                # Završeno vozilo ne obrađujemo ponovno
                if track_id not in zavrseni_id:
                    ...
                    for naziv_trake, oznake in trake.items():
                        ...
                        if presao_liniju(
                            ...
                            # Prolaz se potvrđuje samo ako je isti ID
                            # prethodno prešao L1 iste prometne trake.
                            if (
                                kljuc_vozila_i_trake
                                in aktivacije_l1
                            ):
                                ...
                                if razlika_vremena > 0:
                                    ...
                                    # Poruka ostaje na videu dvije sekunde
                                    zadnja_poruka_do_framea = (
                                        frame_broj + int(fps * 2)
                                    )

                                    print("\n----------------------------------------")
                                    print("ISPRAVAN PROLAZ")
                                    print("----------------------------------------")
                                    print(
    ...
    ukupno_ispravnih = sum(brojaci.values())

    cv2.putText(
        annotated_frame,
        f"Ukupno ispravnih prolazaka: {ukupno_ispravnih}",
        (10, 160),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.75,
        (255, 255, 255),
...
print("\n========================================")
print("       REZULTAT TESTA 06")
print("========================================")
print("Status        : Uspješno završeno")
print(f"Ulazni video  : {ulazni_video}")
print("Model         : yolo11n.pt")
print("Metoda        : Redoslijed Linija 1 -> Linija 2")
